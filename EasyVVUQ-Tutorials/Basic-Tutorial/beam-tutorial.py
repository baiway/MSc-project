...
# Define parameter space. See: https://openturns.github.io/openturns/latest/usecases/use_case_deflection_tube.html 
def define_params():
    return {
...
# Define distributions to sample over for each varying quantity (here just 4 of 6)
def define_vary():
    return {
        "F":   cp.Normal(1, 0.1),
...
def run_campaign(pce_order=2, use_files=False):
    ...
    # Set up a fresh campaign called "beam_pce"
...
if __name__ == '__main__':
    ...
    for pce_order in range(1, max_order):
        ...
        R[pce_order]['times'], 
        R[pce_order]['order'], 
        R[pce_order]['number_of_samples']) = run_campaign(pce_order=pce_order, use_files=True)
